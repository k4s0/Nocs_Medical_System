...
if (isPlayer _healer) then {
	if (_healer distance _injuredperson > 2.5) exitWith {
		if (isPlayer _healer) then {[format ["%1 è troppo lontano per essere soccorso." call tcb_ais_font, name _injuredperson],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText};
	};
};
...
if (!_ableToHeal) exitWith {
	if (tcb_ais_medical_education >= 2) then {
		if (isPlayer _healer) then {[format ["Solo i medici sono addestrati per questa procedura!" call tcb_ais_font],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText};
	} else {
		if (isPlayer _healer) then {[format ["Hai bisogno di equipaggiamento medico per questa procedura!" call tcb_ais_font],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText};
	};
	if (!isPlayer _healer) then {
		_healer stop false;
...
while {
	...
	sleep 0.5;
	if (isPlayer _healer) then {["Soccorrendo ",((time - _time) / (_damage)) min 1] spawn tcb_fnc_progressbar};
};
...
if (!alive _injuredperson) exitWith {["Non è più con noi." call tcb_ais_font,0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText};
...
if (!tcb_healerStopped) then {
	...
	["Hai interrotto il soccorso." call tcb_ais_font,0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText;
};
