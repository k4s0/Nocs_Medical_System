...
if (_injuredperson distance _dragger > 3) exitWith {[format ["%1 è fuori portata per essere trascinato." call tcb_ais_font, name _injuredperson],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText};
...
if (_dragger call tcb_fnc_checklauncher) exitWith {[format ["Non puoi caricare nessuno sulle spalle se hai un lanciarazzi" call tcb_ais_font],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText};
