...
"tcb_ais_in_agony" addPublicVariableEventHandler {
	...
	if (playerSide == _side) then {
		if (_in_agony) then {
			...
			if (local player) then {[side _unit,"HQ"] sideChat format ["%1 è a terra e necessita di aiuto %2!",name _unit, mapGridPosition _unit]};

			if (isNil {_unit getVariable ["fa_action", Nil]}) then {
				_fa_action = _unit addAction [format["<t color='#F00000'>Cura %1</t>",name _unit],{_this spawn tcb_fnc_firstAid},_unit,100,false,true,"",
			...
			if (isNil {_unit getVariable ["drag_action", Nil]}) then {
				_drag_action = _unit addAction [format["<t color='#FC9512'>Trascina %1</t>",name _unit],{_this spawn tcb_fnc_drag},_unit,99,false,true,"",
			...
			if (tcb_ais_medical_education >= 1) then {
				if (isNil {_unit getVariable ["help_action", Nil]}) then {
					_help_action = _unit addAction [format["<t color='#FC9512'>Premi sulla ferita</t>",name _unit],{_this spawn tcb_fnc_help},_unit,98,false,true,"",
						"{not isNull (_target getVariable _x)} count ['healer','dragger','helper'] == 0 && {alive _target} && {vehicle _target == _target}
					"];
